...
def crop_pdf(input_pdf, output_pdf, *, mode=None, w1=None, w2=None, h1=None, h2=None, pages="all"):
    """
    ...
    if mode:
        ...
        if mode == "left":
            ...
            raise ValueError(f"Unknown mode: '{mode}'. Use 'left', 'right', 'top', or 'bottom'.")
    else:
        # Validate fractional inputs
        if not (0 <= w1 < w2 <= 1):
            raise ValueError(f"Invalid width: w1={w1}, w2={w2}. Must satisfy 0 ≤ w1 < w2 ≤ 1.")
        if not (0 <= h1 < h2 <= 1):
            raise ValueError(f"Invalid height: h1={h1}, h2={h2}. Must satisfy 0 ≤ h1 < h2 ≤ 1.")
...
"""
    ...
    parser.add_argument("--mode", choices=["left", "right", "top", "bottom"],
    ...
    parser.add_argument("--h2", type=float, help="Bottom boundary as fraction (0.0–1.0)")
    ...
    if args.mode is not None:
        ...
        crop_pdf(args.input, args.output, w1=args.w1, w2=args.w2, h1=args.h1, h2=args.h2, pages=args.pages)
    else:
        parser.error("Either --mode must be specified, or all of --w1, --w2, --h1, and --h2.")
